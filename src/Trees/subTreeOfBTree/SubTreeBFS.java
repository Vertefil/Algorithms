...
public class SubTreeBFS {
     ...
     *
     *  Вызываем саму функцию рекурсивно по уровням у левого, потом у правого ребёнка
    ...
    public static boolean isSameTree(TreeNode p, TreeNode q) {
        ...
        while(!q1.isEmpty() && !q2.isEmpty()) {
            for (int i = q1.size(); i > 0; i--) {
                TreeNode nodeOne = q1.poll();
                TreeNode nodeTwo = q2.poll();
                if (nodeOne == null && nodeTwo == null) continue;
                if (nodeOne == null || nodeTwo == null || (nodeOne.val != nodeTwo.val)) return false;

                q1.offer(nodeOne.left);
                q1.offer(nodeOne.right);
                q2.offer(nodeTwo.left);
                q2.offer(nodeTwo.right);
            }
        }
        return true;
    }
